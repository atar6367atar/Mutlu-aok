...
<head>
    ...
    <style>
        /* <-- Buraya, sizin verdiğiniz orijinal CSS kodunun aynısı gelecek. -->
        <!-- Kodun kalabalık olmaması için CSS'i kısaltarak gösteriyorum, -->
        <!-- ancak siz kendi CSS'inizi buraya olduğu gibi kopyalayın. -->
        * { margin: 0; padding: 0; box-sizing: border-box; }
        :root { --joker-purple: #8b5cf6; --joker-dark: #2e1065; --joker-darker: #1e0b36; --joker-light: #c4b5fd; --joker-glow: #a78bfa; --joker-green: #10b981; }
        body { font-family: 'Orbitron', sans-serif; background: linear-gradient(135deg, #000, var(--joker-darker), var(--joker-dark)); min-height: 100vh; overflow-x: hidden; position: relative; }
        .floating-card { position: fixed; font-size: 40px; color: var(--joker-purple); opacity: 0.2; pointer-events: none; z-index: 1; animation: float 10s ease-in-out infinite; }
        @keyframes float { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-30px) rotate(10deg); } }
        .card1 { top: 10%; left: 5%; animation-delay: 0s; } .card2 { top: 20%; right: 8%; animation-delay: 2s; } .card3 { bottom: 15%; left: 10%; animation-delay: 4s; } .card4 { bottom: 25%; right: 15%; animation-delay: 6s; } .card5 { top: 50%; left: 50%; animation-delay: 8s; }
        @keyframes glow { 0% { filter: drop-shadow(0 0 5px var(--joker-purple)); } 50% { filter: drop-shadow(0 0 30px var(--joker-light)); } 100% { filter: drop-shadow(0 0 5px var(--joker-purple)); } }
        @keyframes pulse { 0% { transform: scale(1); } 50% { transform: scale(1.05); } 100% { transform: scale(1); } }
        .login-wrapper { min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 20px; position: relative; z-index: 10; }
        .login-card { background: rgba(46, 16, 101, 0.95); backdrop-filter: blur(10px); border-radius: 40px; padding: 50px; width: 100%; max-width: 450px; border: 2px solid var(--joker-purple); box-shadow: 0 0 50px var(--joker-glow); animation: glow 3s ease-in-out infinite; }
        .login-logo { text-align: center; margin-bottom: 40px; }
        .login-logo i { font-size: 80px; color: var(--joker-light); animation: pulse 2s ease-in-out infinite; }
        .login-logo h1 { font-size: 48px; color: white; text-shadow: 0 0 20px var(--joker-purple); margin: 15px 0; }
        .login-input-group { margin-bottom: 25px; }
        .login-input-group input { width: 100%; padding: 18px 25px; background: rgba(255,255,255,0.1); border: 2px solid var(--joker-purple); border-radius: 20px; color: white; font-size: 16px; }
        .login-input-group input:focus { outline: none; border-color: var(--joker-light); box-shadow: 0 0 30px var(--joker-glow); }
        .login-btn { width: 100%; padding: 18px; background: linear-gradient(135deg, var(--joker-purple), var(--joker-light)); border: none; border-radius: 20px; color: white; font-size: 18px; font-weight: 800; cursor: pointer; text-transform: uppercase; letter-spacing: 2px; animation: pulse 2s ease-in-out infinite; }
        .login-btn:hover { transform: translateY(-5px); box-shadow: 0 20px 40px rgba(139, 92, 246, 0.5); }
        .dashboard { padding: 30px; position: relative; z-index: 10; }
        .header { background: rgba(46, 16, 101, 0.95); backdrop-filter: blur(10px); border-radius: 30px; padding: 25px 35px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center; border: 2px solid var(--joker-purple); box-shadow: 0 0 30px var(--joker-glow); }
        .header h1 { color: white; font-size: 32px; display: flex; align-items: center; gap: 15px; }
        .header h1 i { color: var(--joker-light); animation: pulse 2s ease-in-out infinite; }
        .badge { background: linear-gradient(135deg, var(--joker-purple), var(--joker-light)); padding: 10px 25px; border-radius: 40px; color: white; font-weight: 700; }
        .logout-btn { background: rgba(239, 68, 68, 0.3); color: white; border: 1px solid #ef4444; padding: 10px 25px; border-radius: 40px; text-decoration: none; transition: all 0.3s; }
        .logout-btn:hover { background: #ef4444; transform: translateY(-3px); }
        .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 30px; }
        .card { background: rgba(46, 16, 101, 0.9); backdrop-filter: blur(10px); border-radius: 20px; padding: 20px; border: 2px solid var(--joker-purple); transition: all 0.3s; cursor: pointer; text-align: center; }
        .card:hover { transform: translateY(-10px); border-color: var(--joker-light); box-shadow: 0 20px 40px rgba(139, 92, 246, 0.3); }
        .card-icon { width: 50px; height: 50px; background: linear-gradient(135deg, var(--joker-purple), var(--joker-light)); border-radius: 15px; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px; }
        .card-icon i { font-size: 24px; color: white; }
        .card h3 { color: white; font-size: 16px; }
        .query-box { background: rgba(46, 16, 101, 0.95); backdrop-filter: blur(10px); border-radius: 30px; padding: 30px; margin-bottom: 30px; border: 2px solid var(--joker-purple); }
        .query-header { display: flex; align-items: center; gap: 20px; margin-bottom: 25px; padding-bottom: 20px; border-bottom: 2px solid var(--joker-purple); }
        .query-header i { font-size: 40px; color: var(--joker-light); animation: pulse 2s ease-in-out infinite; }
        .query-header h2 { color: white; font-size: 24px; }
        .input-group { display: flex; gap: 15px; margin-bottom: 15px; }
        .input-group input { flex: 1; padding: 18px 25px; background: rgba(255,255,255,0.1); border: 2px solid var(--joker-purple); border-radius: 20px; color: white; font-size: 16px; }
        .input-group button { padding: 18px 35px; background: linear-gradient(135deg, var(--joker-purple), var(--joker-light)); border: none; border-radius: 20px; color: white; font-weight: 800; cursor: pointer; transition: all 0.3s; }
        .input-group button:hover { transform: translateY(-5px); box-shadow: 0 20px 40px rgba(139, 92, 246, 0.5); }
        .input-group button:disabled { opacity: 0.5; cursor: not-allowed; }
        .example { color: var(--joker-light); font-size: 14px; display: flex; align-items: center; gap: 10px; }
        .loader { display: none; text-align: center; padding: 40px; }
        .spinner { width: 50px; height: 50px; border: 4px solid var(--joker-purple); border-top-color: var(--joker-light); border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 15px; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .result { background: rgba(0,0,0,0.3); border-radius: 20px; padding: 25px; margin-top: 25px; border: 2px solid var(--joker-purple); display: none; }
        .result-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
        .result-header h3 { color: var(--joker-green); display: flex; align-items: center; gap: 10px; }
        .result-actions { display: flex; gap: 10px; }
        .result-actions button { padding: 8px 15px; background: rgba(255,255,255,0.1); border: 2px solid var(--joker-purple); border-radius: 10px; color: white; cursor: pointer; transition: all 0.2s; }
        .result-actions button:hover { background: var(--joker-purple); }
        .result-content { background: rgba(0,0,0,0.5); border-radius: 15px; padding: 20px; font-family: monospace; font-size: 14px; color: var(--joker-light); max-height: 400px; overflow-y: auto; white-space: pre-wrap; word-break: break-word; }
        .result-content.small { background: rgba(16, 185, 129, 0.1); border-left: 5px solid var(--joker-green); }
        .recent { background: rgba(46, 16, 101, 0.95); backdrop-filter: blur(10px); border-radius: 30px; padding: 30px; border: 2px solid var(--joker-purple); }
        .recent-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; }
        .recent-header h2 { color: white; display: flex; align-items: center; gap: 10px; }
        .clear-btn { padding: 10px 20px; background: rgba(239, 68, 68, 0.3); border: 1px solid #ef4444; border-radius: 15px; color: white; cursor: pointer; }
        .clear-btn:hover { background: #ef4444; }
        .recent-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 15px; }
        .recent-item { background: rgba(0,0,0,0.3); border-radius: 15px; padding: 15px; cursor: pointer; transition: all 0.3s; border: 1px solid transparent; }
        .recent-item:hover { background: rgba(139, 92, 246, 0.3); transform: translateX(10px); border-color: var(--joker-purple); }
        .recent-type { background: linear-gradient(135deg, var(--joker-purple), var(--joker-light)); color: white; padding: 4px 12px; border-radius: 20px; font-size: 11px; display: inline-block; margin-bottom: 8px; }
        .recent-param { color: white; font-weight: 700; margin-bottom: 5px; }
        .recent-time { color: rgba(255,255,255,0.5); font-size: 11px; }
    </style>
</head>
<body>
